...
class mcDataset(TaskDataset):
    ...
    prompt_input = PROMPT_DICT["prompt_mc_input"]
    prompt_no_input = PROMPT_DICT["prompt_mc_no_input"]

    @classmethod
    def load_dataset(cls, data_path, tokenizer, max_length):
        train_file = open(os.path.join(data_path, "train2.json"), encoding='utf-8')
        test_file = open(os.path.join(data_path, "test2.json"), encoding='utf-8')
        train_data = json.load(train_file)
        test_data = json.load(test_file)
        train_file.close()
        test_file.close()
...
def main():
    ...
    train_dataset, test_dataset = mcDataset.load_dataset(
        data_path="/d2/mxy/W-LoRA/data/Domains-Based/med/mc",
        tokenizer=tokenizer,
        max_length=512
    )
